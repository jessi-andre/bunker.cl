...
const getSupabaseAdmin = () => {
  if (!_supabaseAdmin) {
    ...
    _supabaseAdmin = createClient(supabaseUrl, serviceRole);
  }
...
const getSubdomainSlugFromHost = (host = "") => {
...
const getCompanyByReqHost = async (req) => {
  ...
  const slug = getSubdomainSlugFromHost(host);
  if (!slug) return null;
...
function createRequestId(req) {
  ...
  if (fromHeader) {
    ...
    return String(fromHeader).replace(/[^\x20-\x7E]/g, "").slice(0, 64);
  }
...
async function requireAuth(req, res, opts = {}) {
  ...
  if (!Number.isFinite(expiresAtMs) || expiresAtMs <= now) {
    ...
    json(res, 401, { error: "Session expired", request_id: requestId });
    return null;
  }
...
async function requireAuthAndTenant(req, opts = {}) {
  const { allowInactiveSubscription = false } = opts;
  ...
  const { data: session, error } = await supabase
    .from("admin_sessions")
    .select("id, admin_id, company_id, expires_at, token_hash, created_at")
    .eq("token_hash", tokenHash)
    .maybeSingle();

  if (error) {
    const err = new Error("Failed to validate session");
    err.status = 500;
    throw err;
  }
  ...
  if (!allowInactiveSubscription) {
    ...
    if (subError) {
      const err = new Error("Failed to read subscription status");
      err.status = 500;
      throw err;
    }

    const subscriptionStatus = String(subRow?.subscription_status || "").toLowerCase();
    if (subscriptionStatus !== "active") {
      const err = new Error("SUBSCRIPTION_INACTIVE");
      err.status = 402;
      throw err;
    }
  }

  // Check admin-level session revocation
  const sessionCreatedAtMs = new Date(session.created_at || 0).getTime();
  try {
    const { data: adminRow } = await supabase
      .from("company_admins")
    ...
    const adminRevokedAtMs = new Date(adminRow?.sessions_revoked_at || 0).getTime();
    if (adminRevokedAtMs > 0 && sessionCreatedAtMs < adminRevokedAtMs) {
      await supabase.from("admin_sessions").delete().eq("id", session.id);
      const err = new Error("No session");
      err.status = 401;
      throw err;
  ...
  // Check company-level session revocation
  try {
    const { data: companyRow } = await supabase
      .from("companies")
      .select("sessions_revoked_at")
    ...
    const companyRevokedAtMs = new Date(companyRow?.sessions_revoked_at || 0).getTime();
    if (companyRevokedAtMs > 0 && sessionCreatedAtMs < companyRevokedAtMs) {
      await supabase.from("admin_sessions").delete().eq("id", session.id);
      const err = new Error("No session");
      err.status = 401;
      throw err;
